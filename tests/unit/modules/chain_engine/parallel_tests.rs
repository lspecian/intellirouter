...
async fn test_nested_parallel_execution() {
    ...
    let inner_parallel_steps = vec!["step3".to_string(), "step4".to_string()];

    let inner_parallel_step = create_test_step(
        "inner_parallel",
        StepType::Parallel {
            steps: inner_parallel_steps.clone(),
            wait_for_all: true,
        },
    );

    let outer_parallel_steps = vec!["step2".to_string(), "inner_parallel".to_string()];

    let outer_parallel_step = create_test_step(
        "outer_parallel",
        StepType::Parallel {
            steps: outer_parallel_steps.clone(),
            wait_for_all: true,
        },
    );
    ...
    chain
        .steps
        .insert(outer_parallel_step.id.clone(), outer_parallel_step);
    chain
        .steps
        .insert(inner_parallel_step.id.clone(), inner_parallel_step);
    chain.steps.insert(step2.id.clone(), step2);
    chain.steps.insert(step3.id.clone(), step3);
    chain.steps.insert(step4.id.clone(), step4);
    chain.steps.insert(step5.id.clone(), step5);
    ...
    // Add dependencies
    chain.dependencies.push(StepDependency {
        dependent_step: "outer_parallel".to_string(),
        dependency_type: DependencyType::Simple {
            required_step: "step1".to_string(),
        },
    });
    ...
    chain.dependencies.push(StepDependency {
        dependent_step: "step5".to_string(),
        dependency_type: DependencyType::Simple {
            required_step: "outer_parallel".to_string(),
        },
    });
