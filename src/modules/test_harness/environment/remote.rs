...
//!
//! This module provides a remote environment implementation for testing.
...
use super::config::{EnvironmentConfig, EnvironmentType, RemoteConfig, ResourceRequirements};
...
pub struct RemoteEnvironment {
    ...
    /// Running services
    running_services: HashMap<String, String>,
...
impl RemoteEnvironment {
    ...
    pub async fn new(
        ...
        env_vars.extend(remote_config.env_vars.clone());
...
impl Environment for RemoteEnvironment {
    ...
    async fn setup(&mut self) -> Result<(), TestHarnessError> {
        info!("Setting up remote environment: {}", self.name());
        warn!("Remote environment is not fully implemented yet");
    ...
    async fn teardown(&mut self) -> Result<(), TestHarnessError> {
        info!("Tearing down remote environment: {}", self.name());
        warn!("Remote environment is not fully implemented yet");
    ...
    async fn take_snapshot(&self, _name: &str) -> Result<(), TestHarnessError> {
        ...
        Err(TestHarnessError::EnvironmentError(
            "Snapshots are not supported for remote environments".to_string(),
        ))
    }
